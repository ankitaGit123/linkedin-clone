...
// import { setTokenIsThere } from '@redux/reducer/authReducer';
...
export default function Dashboard() {
    ...
    console.log("postState", authState)

    // Check token and set isTokenThere
    useEffect(() => {
        console.log("useEffect: Check token and set isTokenThere", { isTokenThere: authState.isTokenThere });
    ...
    // Fetch posts and user info only if not already fetched
    useEffect(() => {
        console.log("useEffect: Fetch posts and user info", { isTokenThere: authState.isTokenThere });
        const token = localStorage.getItem('token');
        console.log("object", token)
        if (authState?.isTokenThere && token && !postState?.postFetched) {
            dispatch(getAllPosts());
        }
        if (authState?.isTokenThere && token && !authState.user) {
            dispatch(getAboutUser({ token }));
        }
    }, [authState?.isTokenThere, dispatch, postState.postFetched, authState.user]);

    // Fetch all users only once
    useEffect(() => {
        console.log("useEffect: Fetch all users", { all_profiles_fetched: authState.all_profiles_fetched });
    ...
    const handleUpload = async () => {
        ...
        setPostContent("");
        setFileContent(null);
        // No need to dispatch getAllPosts here if your reducer updates the posts array correctly after creation
    }
                                ...
                                <div className={styles?.scrollComponent}>
                                    ...
                                    <div className={styles?.wrapper}>
                                        ...
                                        <div className={styles?.createPostContainer}>
                                            ...
                                            />
                                            <textarea onChange={(e) => setPostContent(e.target.value)} value={postContent} placeholder={"What is on your mind"} className={styles.textAreaOfContent} name="" id=""> </textarea>
                                                ...
                                                <div key={post._id} className={styles.singleCard}>
                                                    ...
                                                    <div className={styles.singleCard_profileContainer}>
                                                        ...
                                                        <div>
                                                            ...
                                                            <p style={{ color: "grey" }}> {post.userId.email}</p>
                                                            <p styles={{ paddingTop: "1.3rem" }}>{post.body}</p>
